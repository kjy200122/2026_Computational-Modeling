...
pars_mle <- round(cbind(mle_model_pow1$par, mle_model_pow2$par, mle_model_exp1$par, mle_model_exp2$par, mle_model_expow$par, mle_model_hyp1$par, mle_model_hyp2$par),3)
...
p_hyp2 <- parm_hyp2[1]/(1+x*parm_hyp2[2])


plot(x, p_pow2, ylim=c(0,1), xlab='Time t', ylab='Proportion Correct', 
     main="MLE results", type='l', lwd=4)
lines(x, p_exp1, lwd=2, col='orange')
lines(x, p_exp2, lwd=2, lty='dashed', col='red')
lines(x, p_expow, lwd=2.5, col='violet')
lines(x, p_hyp1, lwd=3, col='yellowgreen')
lines(x, p_hyp2, lwd=2, lty='dashed', col='navy')
lines(x, p_pow1, lwd=3, lty='dotted', col='cyan')
points(t_int, p_corr, pch=19, cex=1.5)
text = c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
ltys = c('dotted', 'solid','solid', 'dashed', 'solid', 'solid', 'dotted')
cols = c('cyan', 'black', 'orange', 'red', 'violet', 'yellowgreen', 'navy')
