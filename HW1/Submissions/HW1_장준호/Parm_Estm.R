...
param_pow1_low <- c(0); param_pow1_up <- c(5);  # lower and upper bounds of POW2 model (0<b<5)
param_pow2_low <- c(0, 0); param_pow2_up <- c(1, 5);  # lower and upper bounds of POW2 model (0<a<1, 0<b<5)
param_exp1_low <- c(0); param_exp1_up <- c(5);  # lower and upper bounds of EXP2 model (0<b<5)
param_exp2_low <- c(0, 0); param_exp2_up <- c(1, 5);  # lower and upper bounds of POW2 model (0<a<1, 0<b<5)
param_expow_low <- c(0, 0, 0); param_expow_up <- c(1, 5, 5);  # lower and upper bounds of POW2 model (0<a<1, 0<b<5, 0<c<5)
param_hyp1_low <- c(0); param_hyp1_up <- c(1);  # lower and upper bounds of EXP2 model (0<b<1)
param_hyp2_low <- c(0, 0); param_hyp2_up <- c(1, 1);  # lower and upper bounds of EXP2 model (0<a<1, 0<b<1)
...
p_prd_pow2 <- parm_pow2[1]*(1+t_int)^(-parm_pow2[2])
...
# For cbind / pars_mle
parm_pow1_pad <- c(mle_model_pow1$par, NA, NA)
parm_pow2_pad <- c(mle_model_pow2$par, NA)
parm_exp1_pad <- c(mle_model_exp1$par, NA, NA)
parm_exp2_pad <- c(mle_model_exp2$par, NA)
parm_expow_pad <- c(mle_model_expow$par)
parm_hyp1_pad <- c(mle_model_hyp1$par, NA, NA)
parm_hyp2_pad <- c(mle_model_hyp2$par, NA)

# Generate summary
minus_loglik_MLE = round(c(mle_model_pow1$value, mle_model_pow2$value, mle_model_exp1$value, mle_model_exp2$value, mle_model_expow$value, mle_model_hyp1$value, mle_model_hyp2$value), 3)
...
pars_mle <- round(cbind(parm_pow1_pad, parm_pow2_pad, parm_exp1_pad, parm_exp2_pad, parm_expow_pad, parm_hyp1_pad, parm_hyp2_pad), 3)
dimnames(pars_mle) = list(c('par1', 'par2', 'par3'),c("POW1", "PO2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2"))
...
p_pow2 <- parm_pow2[1]*(1+x)^(-parm_pow2[2])
...
lines(x, p_pow1, lwd=2, lty=1, col='blue')
lines(x, p_pow2, lwd=2, lty=1, col='black')
lines(x, p_exp1, lwd=2, lty=2, col='green')
lines(x, p_exp2, lwd=2, lty=2, col='red')
lines(x, p_expow, lwd=2, lty=3, col='purple')
lines(x, p_hyp1, lwd=2, lty=4, col='orange')
lines(x, p_hyp2, lwd=2, lty=4, col='brown')
points(t_int, p_corr, pch=19, cex=1.5)
text = c("POW1", "PO2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
ltys = c('solid','dashed')
cols = c('blue', 'black', 'green', 'red', 'purple', 'orange', 'brown')
legend("topright", names, lwd=2, cex=0.6, lty=ltys, col=cols)
