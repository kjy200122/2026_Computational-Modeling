...
p_prd_hyp2 <- parm_hyp2[1] / (1 + parm_hyp2[1]*t_int)
...
pars_mle <- round(cbind(c(mle_model_pow1$par, NA, NA),
                        c(mle_model_pow2$par, NA),
                        c(mle_model_exp1$par, NA, NA),
                        c(mle_model_exp2$par, NA),
                        c(mle_model_expow$par),
                        c(mle_model_hyp1$par, NA, NA),
                        c(mle_model_hyp2$par, NA)), 3)
...
mle_sorted = mle_summary[order(mle_summary[, 3], decreasing = TRUE), ]
mle_sorted
...
p_hyp2 <- parm_hyp2[1] / (1 + parm_hyp2[1]*x)

plot(x, p_pow1, ylim=c(0,1), xlab='Time t', ylab='Proportion Correct', 
     main="MLE results", type='l', lwd=2)
lines(x, p_pow2, lwd=2, lty='dashed', col='gray')
lines(x, p_exp1, lwd=2, lty='dotted', col='blue')
lines(x, p_exp2, lwd=2, lty='dotdash', col='turquoise')
lines(x, p_expow, lwd=2, lty='longdash', col='green')
lines(x, p_hyp1, lwd=2, lty='twodash', col='yellow')
lines(x, p_hyp2, lwd=2, lty=44, col='orange')

points(t_int, p_corr, pch=19, cex=1.5)

text = c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
ltys = c('solid','dashed','dotted','dotdash','longdash','twodash',lty=44)
cols = c('black','gray','blue','turquoise','green','red','orange')
