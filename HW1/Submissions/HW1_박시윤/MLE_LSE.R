...
lse_pow1 <- function(param, int=t_int, y=p_corr)  {
  p <- (1+int)^(-param[1])
  sum((y-p)^2)
}

lse_pow2 <- function(param, int=t_int, y=p_corr)  {
  p <- param[1]*(1+int)^(-param[2])
  sum((y-p)^2)
}

lse_exp1 <- function(param, int=t_int, y=p_corr)  {
  p <- exp((-param[1])*int)
  sum((y-p)^2)
}

lse_exp2 <- function(param, int=t_int, y=p_corr)  {
  p <- param[1]*exp((-param[2])*int)
  sum((y-p)^2)
}

lse_expow <- function(param, int=t_int, y=p_corr)  {
  p <- param[1]*exp((-param[2])*int)*(1+int)^(-param[3])
  sum((y-p)^2)
}

lse_hyp1 <- function(param, int=t_int, y=p_corr)  {
  p <- 1/(1+param[1]*int)
  sum((y-p)^2)
}

lse_hyp2 <- function(param, int=t_int, y=p_corr)  {
  p <- param[1]/(1+param[2]*int)
  sum((y-p)^2)
}
