...
param_pow1_low <- c(0); param_pow1_up <- c(5);  # lower and upper bounds of POW1 model (0<b<5)
param_pow2_low <- c(0, 0); param_pow2_up <- c(1, 5);  # lower and upper bounds of POW2 model (0<a<1, 0<b<5)
param_exp1_low <- c(0); param_exp1_up <- c(5);  # lower and upper bounds of EXP1 model (0<b<5)
param_exp2_low <- c(0, 0); param_exp2_up <- c(1, 5);  # lower and upper bounds of EXP2 model (0<a<1, 0<b<5)
param_expow_low <- c(0, 0, 0); param_expow_up <- c(1, 5, 5);  # lower and upper bounds of EXPOW model (0<a<1, 0<b<5, 0<c<5)
...
p_prd_expow <- parm_expow[1]*exp(-parm_expow[2]*t_int)*(1+t_int)^(-parm_expow[3])
...
pars_mle_1 <- round(cbind(mle_model_pow1$par, mle_model_exp1$par, mle_model_hyp1$par),3)
dimnames(pars_mle_1) = list(c('b'), c('POW1', 'EXP1', 'HYP1'))

pars_mle_2 <- round(cbind(mle_model_pow2$par, mle_model_exp2$par, mle_model_hyp2$par),3)
dimnames(pars_mle_2) = list(c('a', 'b'), c('POW2', 'EXP2', 'HYP2'))

pars_mle_3 <- round(cbind(mle_model_expow$par),3)
dimnames(pars_mle_3) = list(c('a', 'b', 'c'), c('EXPOW'))
...
plot(x, p_pow1, ylim=c(0,1), xlab='Time t', ylab='Proportion Correct', 
     main="MLE results", type='l', lwd=4, col='#929292')
lines(x, p_pow2, lwd=2, lty='dashed', col='#1393FD',)
lines(x, p_exp1, lwd=2, lty='solid', col='#18D85F')
lines(x, p_exp2, lwd=2, lty='dashed', col='#FB2E46')
lines(x, p_expow, lwd=2, lty='dotted', col='#FBD92E', alpha=0.5)
lines(x, p_hyp1, lwd=2, lty='solid', col='#FF8127')
lines(x, p_hyp2, lwd=2, lty='dashed', col='#EA69FF')

points(t_int, p_corr, pch=19, cex=1.5)
text = c("POW1","POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
ltys = c("solid",'dashed','solid','dashed', 'dotted', 'solid', 'dashed')
cols = c('#929292','#1393FD', '#18D85F', '#FB2E46', '#FBD92E', '#FF8127', '#EA69FF')
legend(12, 1, text, lwd=2, cex=1, lty=ltys, col=cols)
...
parm_pow1 <- lse_model_pow1$par
parm_pow2 <- lse_model_pow2$par
parm_exp1 <- lse_model_exp1$par
parm_exp2 <- lse_model_exp2$par
parm_expow <- lse_model_expow$par
parm_hyp1 <- lse_model_hyp1$par
parm_hyp2 <- lse_model_hyp2$par
...
rmsd_lse = round(c(lse_model_pow1$value, lse_model_pow2$value, 
                           lse_model_exp1$value, lse_model_exp2$value,
                           lse_model_expow$value,
                           lse_model_hyp1$value, lse_model_hyp2$value), 3)
r2_lse <- round(c(r2_pow1, r2_pow2, r2_exp1, r2_exp2, r2_expow, r2_hyp1, r2_hyp2), 3)  # round off the value, up to 3 digits
names = c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")

pars_lse_1 <- round(cbind(lse_model_pow1$par, lse_model_exp1$par, lse_model_hyp1$par),3)
dimnames(pars_lse_1) = list(c('b'), c('POW1', 'EXP1', 'HYP1'))

pars_lse_2 <- round(cbind(lse_model_pow2$par, lse_model_exp2$par, lse_model_hyp2$par),3)
dimnames(pars_lse_2) = list(c('a', 'b'), c('POW2', 'EXP2', 'HYP2'))

pars_lse_3 <- round(cbind(lse_model_expow$par),3)
dimnames(pars_lse_3) = list(c('a', 'b', 'c'), c('EXPOW'))


lse_summary = data.frame(Models = names, loglik = rmsd_lse, r2 = r2_lse)
...
# Plot the lse results using simple R graphics. Use ggplots for fancy graphs
x <- seq(0,20, 0.05)
p_pow1 <- (x+1)^(-parm_pow1[1])
p_pow2 <- parm_pow2[1]*(x+1)^(-parm_pow2[2])
p_exp1 <- exp(-parm_exp2[1]*x)
...
plot(x, p_pow1, ylim=c(0,1), xlab='Time t', ylab='Proportion Correct', 
     main="LSE results", type='l', lwd=4, col='#929292')
lines(x, p_pow2, lwd=2, lty='dashed', col='#1393FD',)
lines(x, p_exp1, lwd=2, lty='solid', col='#18D85F')
lines(x, p_exp2, lwd=2, lty='dashed', col='#FB2E46')
lines(x, p_expow, lwd=2, lty='dotted', col='#FBD92E', alpha=0.5)
