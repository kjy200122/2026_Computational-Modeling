...
p_prd_hyp1 <- (1+parm_hyp1[1]*t_int)^(-1)
p_prd_hyp2 <- parm_hyp2[1]*(1+parm_hyp2[2]*t_int)^(-1)
...
minus_loglik_MLE = round(c(mle_model_pow1$value, mle_model_pow2$value, 
                           mle_model_exp1$value, mle_model_exp2$value, 
                           mle_model_expow$value, mle_model_hyp1$value, 
...
pars_mle <- round(cbind(c(mle_model_pow1$par,NA,NA), c(mle_model_pow2$par,NA), c(mle_model_exp1$par,NA,NA), c(mle_model_exp2$par,NA), mle_model_expow$par, c(mle_model_hyp1$par,NA,NA), c(mle_model_hyp2$par,NA)), 3)
...
plot(x, p_pow1, ylim=c(0,1), xlab='Time t', ylab='Proportion Correct',
     main="MLE results", type='l', lwd=2, col='black')
lines(x, p_pow2,  lwd=2, lty='solid',  col='red')
lines(x, p_exp1,  lwd=2, lty='dotted',  col='blue')
lines(x, p_exp2,  lwd=2, lty='dotted', col='green3')
lines(x, p_expow, lwd=2, lty='dashed',col='purple')
lines(x, p_hyp1,  lwd=2, lty='dotdash', col='orange')
lines(x, p_hyp2,  lwd=2, lty='dotdash',   col='brown')
points(t_int, p_corr, pch=19, cex=1.5)
text = c("POW1","POW2","EXP1","EXP2","EXPOW","HYP1","HYP2")
ltys = c('solid', 'solid', 'dotted','dotted', 'dashed', 'dotdash', 'dotdash')
cols = c('black','red','blue','green3','purple','orange','brown')
legend(12, 1, text, lwd=2, cex=0.7, lty=ltys, col=cols)
...
# Call general purpose optimization routine
...
# Try many different inits to escape from the local minima
for (i in 1:100) {
  param1_init <- runif(1); param2_init <- runif(2); param3_init <- runif(3);
...
# LSE predictions
p_lse_prd_pow1 <- (t_int+1)^(-parm_lse_pow1[1])
p_lse_prd_pow2 <- parm_lse_pow2[1]*(t_int+1)^(-parm_lse_pow2[2])
p_lse_prd_exp1 <- exp(-parm_lse_exp1[1]*t_int)
p_lse_prd_exp2 <- parm_lse_exp2[1]*exp(-parm_lse_exp2[2]*t_int)
p_lse_prd_expow <- parm_lse_expow[1]*exp(-parm_lse_expow[2]*t_int)*(t_int+1)^(-parm_lse_expow[3])
p_lse_prd_hyp1 <- (1+parm_lse_hyp1[1]*t_int)^(-1)
p_lse_prd_hyp2 <- parm_lse_hyp2[1]*(1+parm_lse_hyp2[2]*t_int)^(-1)

# Proportion of the explained variances for each model
r2_lse_pow1 = 1-sum((p_corr-p_lse_prd_pow1)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_pow2 = 1-sum((p_corr-p_lse_prd_pow2)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_exp1 = 1-sum((p_corr-p_lse_prd_exp1)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_exp2 = 1-sum((p_corr-p_lse_prd_exp2)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_expow = 1-sum((p_corr-p_lse_prd_expow)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_hyp1 = 1-sum((p_corr-p_lse_prd_hyp1)^2)/sum((p_corr-mean(p_corr))^2)
r2_lse_hyp2 = 1-sum((p_corr-p_lse_prd_hyp2)^2)/sum((p_corr-mean(p_corr))^2)

# Generate summary
sse_LSE = round(c(lse_model_pow1$value, lse_model_pow2$value,
                  lse_model_exp1$value, lse_model_exp2$value,
                  lse_model_expow$value, lse_model_hyp1$value,
                  lse_model_hyp2$value), 3)
r2_lse <- round(c(r2_lse_pow1, r2_lse_pow2, r2_lse_exp1, r2_lse_exp2, r2_lse_expow, r2_lse_hyp1, r2_lse_hyp2), 3)
names_lse = c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
pars_lse <- round(cbind(c(lse_model_pow1$par,NA,NA), c(lse_model_pow2$par,NA), c(lse_model_exp1$par,NA,NA), c(lse_model_exp2$par,NA), lse_model_expow$par, c(lse_model_hyp1$par,NA,NA), c(lse_model_hyp2$par,NA)), 3)
dimnames(pars_lse) = list(c('par1','par2','par3'), c('POW1','POW2','EXP1','EXP2','EXPOW','HYP1','HYP2'))

lse_summary = data.frame(Models = names_lse, SSE = sse_LSE, r2 = r2_lse)

# Print LSE results
print('- LSE results ------------')
print(lse_summary, 4)

print('- LSE Best-fit parameters --------')
print(pars_lse, 4)
