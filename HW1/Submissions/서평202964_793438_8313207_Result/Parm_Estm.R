...
param_hyp1_low <- c(0);param_hyp1_up <- c(5);
...
pars_mle <- round(cbind(
  c(mle_model_pow1$par,  NA, NA)[1:3],
  c(mle_model_pow2$par,  NA, NA)[1:3],
  c(mle_model_exp1$par,  NA, NA)[1:3],
  c(mle_model_exp2$par,  NA, NA)[1:3],
  c(mle_model_expow$par, NA, NA)[1:3],
  c(mle_model_hyp1$par,  NA, NA)[1:3],
  c(mle_model_hyp2$par,  NA, NA)[1:3]
), 3)

dimnames(pars_mle) <- list(
  c("par1", "par2", "par3"),
  c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2")
)
...
plot(x, p_pow1, ylim = c(0, 1), xlab = "Time t", ylab = "Proportion Correct",
     main = "MLE results", type = "l", lwd = 3, col = "black")

lines(x, p_pow2,  lwd = 2, lty = "dashed",  col = "gray")
lines(x, p_exp1,  lwd = 2, lty = "solid",   col = "blue")
lines(x, p_exp2,  lwd = 2, lty = "dashed",  col = "skyblue")
lines(x, p_expow, lwd = 2, lty = "solid",   col = "green")
lines(x, p_hyp1,  lwd = 2, lty = "solid",   col = "red")
lines(x, p_hyp2,  lwd = 2, lty = "dashed",  col = "orange")

points(t_int, p_corr, pch = 19, cex = 1.5)

legend("topright",
       legend = c("POW1", "POW2", "EXP1", "EXP2", "EXPOW", "HYP1", "HYP2"),
       lwd = 2,
       lty = c("solid", "dashed", "solid", "dashed", "solid", "solid", "dashed"),
       col = c("black", "gray", "blue", "skyblue", "green", "red", "orange"),
       cex = 0.9)

# print maximized likelihood values
print("- MLE results ------------")
print(mle_summary, 4)
