...
# Q1.
## (1)
1. run `simulate_hw7_model1.R` and create `simul_data_hw7_model1.txt`
...
# Q4
## (1) 
1. For simulation, `simulate_hw7_model3.R`.
2. For parameter estimation, see `Q4_hw7_model3.R` & `Q4_hw7_model3.stan` in `Q4` folder.

```{r}
# load the output 
load('Q4/output_Q4_model3.RData')
# extract parameter values 
...
## (3) 
1. For simulation, see `simulate_hw7_model4.R`.
2. For parameter estimation, see `Q4_hw7_model4.R` & `Q4_hw7_model4.stan` in `Q4` folder.

```{r}
# load the output 
load('Q4/output_Q4_model4.RData')

# extract parameter values 
parameters <- rstan::extract(output_Q4_model4)
```

## Posterior distributions of group-level parameters
```{r, fig.width=4, fig.height=3}
# group-level parameters
p1 <- draw.post_dist(output_Q4_model4, c('mu_alpha_pos', 'mu_alpha_neg', 'mu_beta', 'sigma')) + 
  labs(title = 'group-level parameters')
p1
```
...
```{r, fig.width=12, fig.height=20}
# individual-level parameters
p1 <- draw.post_dist(output_Q4_model4, 'alpha_pos')
p2 <- draw.post_dist(output_Q4_model4, 'alpha_neg')
p3 <- draw.post_dist(output_Q4_model4, 'beta')
