...
simul_pars <- data.frame(
  alpha_pos = rnorm(num_subjs, 0.20, 0.08),
...
for (i in 1:num_subjs) {
  ...
  alpha_neg <- simul_pars$alpha_neg[i]  # add
...
all_data$choice <- as.integer(all_data$choice) # 경로 문제로 인해서 다음과 같이 int로 변환환
all_data$outcome <- as.integer(all_data$outcome)

write.table(all_data, file = "simul_data_hw6_model1.txt", 
