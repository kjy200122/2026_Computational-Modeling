...
# run!
output = stan("hw6_model1.stan", data = dataList, pars = c("mu_alpha_pos", "mu_alpha_neg", "mu_beta", "alpha_pos", "alpha_neg", "beta"),
              iter = 2000, warmup=1000, chains=2, cores=2)

# variational inference
m = stan_model("hw6_model1.stan")
output_vb = vb(m, data = dataList, pars = c("mu_alpha_pos", "mu_alpha_neg", "mu_beta", "alpha_pos", "alpha_neg", "beta")
)
# traceplot
traceplot(output)
...
print(output, pars="mu_alpha_pos", probs=c(.025,.5,.975))
print(output, pars="mu_alpha_neg", probs=c(.025,.5,.975))
