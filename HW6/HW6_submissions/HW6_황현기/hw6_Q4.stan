// model with alpha_pos, alph_neg, and beta
data {
  int<lower=1> N;
  int<lower=1> T;               
...
model {
  ...
  alpha_pos_pr ~ normal(0,1); // added individual paramter priors to better convergence
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {        
      ...
      if (PE >= 0) {ev[choice[i,t]] = ev[choice[i,t]] + alpha_pos[i] * PE;}
      else {ev[choice[i,t]] = ev[choice[i,t]] + alpha_neg[i] * PE;}
    }
  }
}
