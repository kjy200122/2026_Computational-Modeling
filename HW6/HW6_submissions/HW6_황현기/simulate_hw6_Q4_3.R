...
for (i in 1:num_subjs) {
  ...
  for (t in 1:num_trials)  {
    ...
    # change code so that includes two types of alpha
    if (PE >= 0) {sv[choice] = sv[choice] + alpha_pos * PE}
    else {sv[choice] = sv[choice] + alpha_neg * PE}
...
# Write out data
write.table(all_data, file = "simul_data_hw6_Q4_3.txt", row.names = FALSE, col.names = TRUE, sep = "\t")
