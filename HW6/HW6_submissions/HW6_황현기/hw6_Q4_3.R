...
# read the data file
dat = read.table("simul_data_hw6_Q4_3.txt", header=TRUE, sep="\t")
...
# run!
output_Q4_3 = stan("hw6_Q4_3.stan", data = dataList, pars = c("alpha_pos", "alpha_neg", "beta", "mu_p", "sigma"), #c("mu_alpha", "mu_beta", "alpha", "beta"),
              iter = 2000, warmup=1000, chains=2, cores=2)

# variational inference
# m = stan_model("hw6_Q4_3.stan")
# output_vb = vb(m, data = dataList, pars = c("alpha_pos", "alpha_neg", "beta"), #c("mu_alpha", "mu_beta", "alpha", "beta"),)
# traceplot
traceplot(output_Q4_3, pars = "alpha_pos")
traceplot(output_Q4_3, pars = "alpha_neg")
traceplot(output_Q4_3, pars = "beta")

# print summary
print(output_Q4_3)

# extract Stan fit object (parameters)
parameters <- rstan::extract(output_Q4_3)
