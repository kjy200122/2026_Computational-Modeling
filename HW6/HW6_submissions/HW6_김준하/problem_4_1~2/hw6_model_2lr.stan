...
// Declare all parameters as vectors for vectorizing
  ...
  vector[N] alpha_pos_pr;    // learning rate for positive pe [0, 1] 
  vector[N] alpha_neg_pr;   // learning rate for negative pe [0, 1]
...
model {
  ...
  for (i in 1:N) {
    ...
    for (t in 1:(Tsubj[i])) {        
      ...
      // set alpha according to the sign of PE
      if (PE >= 0)
        alpha = alpha_pos[i];
      else
        alpha = alpha_neg[i];
      ...
      ev[choice[i,t]] = ev[choice[i,t]] + alpha * PE; 
    }
  }
}
