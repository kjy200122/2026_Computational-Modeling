...
#setwd("D:/대학원/2026-봄/수업/임상심리세미나/HW6")
library(rstan)

# read the data file
dat = read.table("simul_data_hw6_model_fake.txt", header=T, sep="\t")
...
# run!
output = stan("hw6_model1.stan", data = dataList, pars = c("alpha", "beta"), #c("mu_alpha", "mu_beta", "alpha", "beta"),
              iter = 2000, warmup=1000, chains=2, cores=2)

#for group
output = stan("hw6_model1.stan", data = dataList, pars = c("mu_p", "sigma", "alpha", "beta"),
iter = 2000, warmup=1000, chains=2, cores=2)
